...
static IR_External_Function check_external_function(Parser *parser, IR_Function *function) {
    ...
    } else if (string_equals_cstr(name, "$memcmp")) {
        if (return_type == i32_type && parameter_count == 3 && parameters[0]->type == any_ptr_type && parameters[1]->type == any_ptr_type && parameters[2]->type == usize_type) {
            return IR_EXTERNAL_FUNCTION__memcmp;
        }
