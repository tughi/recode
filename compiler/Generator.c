...
void generate_module_header(Checked_Module *checked_module, String *output_dir) {
    ...
    while (checked_symbol != NULL) {
        if (checked_symbol->kind == CHECKED_SYMBOL_KIND__IMPORT) {
            Checked_Module *module = ((Checked_Import_Symbol *)checked_symbol)->module;
...
void generate(Checked_Source *checked_source, String *output_dir, bool generate_main) {
    ...
    while (checked_module != NULL) {
        generate_module_header(checked_module, output_dir);
